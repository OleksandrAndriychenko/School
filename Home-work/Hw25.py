class Car():
    FUEL_TYPES = ['бензин', 'дизель', 'електрика', 'гібрид']
    COLORS = []
    NUMBER_OF_CARS = 0
    num = 0

    def __init__(self, model, year, fuel_type, color):
        self.model = model
        self.year = year
        self.color = color
        self.fuel_type = Car.is_valid_fuel_type(fuel_type)
        Car.NUMBER_OF_CARS += 1
        Car.colors_1(color)
        self.number = Car.NUMBER_OF_CARS

    @staticmethod
    def colors_1(color):
        if color in Car.COLORS:
            return Car.COLORS
        else:
            return Car.COLORS.append(color)

    @staticmethod
    def is_valid_fuel_type(fuel_type):
        if fuel_type in Car.FUEL_TYPES:
            return fuel_type
        else:
            return Car.FUEL_TYPES[0]

    def __str__(self):
        return f'модель – {self.model}, рік_випуску – {self.year}, тип_двигуна - {self.fuel_type}, колір - {self.color}'

    @staticmethod
    def get_used_colors():
        return len(Car.COLORS)

    @staticmethod
    def get_number_of_cars():
        return Car.NUMBER_OF_CARS

    @staticmethod
    def numbers():
        Car.num += 1
        return f'{Car.num} from {Car.NUMBER_OF_CARS}'



car_1 = Car('Zaz', 1979, 'дизель', 'black')
car_2 = Car('BMW', 2000, 'бензин', 'red')
car_3 = Car('VOLVO', 2012, 'електрикаcccc', 'black')
car_4 = Car('Mersedes', 2012, 'гібрид', 'black')
print('COLORS:', Car.get_used_colors())
print('NUMBER_OF_CARS:', Car.get_number_of_cars())
for item in (car_1, car_2, car_3, car_4):
    print('item:', item)
    print('numbers:', item.numbers)
